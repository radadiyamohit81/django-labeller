    ...
    var LabelClassEditor = (function () {
        function LabelClassEditor(update_url) {
            ...
            new Sortable(group_container[0], {
                animation: 150,
                onEnd: function (evt) {
                    var end_pos = evt.newIndex;
                    var src_panel = $(evt.item);
                    var params = {
            ...
            for (var i = 0; i < group_table_bodies.length; i++) {
                new Sortable(group_table_bodies[i], {
                    ...
                    onEnd: function (evt) {
                        var end_pos = evt.newIndex;
                        var src_row = $(evt.item);
                        var src_group_id = $(evt.from).closest('.group_panel').data('group-id');
                        var dst_group_id = $(evt.to).closest('.group_panel').data('group-id');
            ...
            new_label_class_button.on("click", function () {
                ...
                var group_id = group_panel.data('group-id');
                var dialog = $("#label_class_create_form");
                var group_id_hid = dialog.find("#class_editor_new_cls_group_id");
                var group_name = group_panel.find(".input_group_name").val();
                var group_name_span = dialog.find("#class_editor_create_label_group_name");
                group_id_hid.val(group_id);
                group_name_span.text(group_name);
            });
        }
        LabelClassEditor.prototype.send_update = function (action, params, on_success) {
            var self = this;
            var post_data = {
                action: action,
        ...
        LabelClassEditor.check_colour = function (col) {
        ...
        LabelClassEditor.colour_regex = /#[A-Fa-f0-9]{6}/;
        return LabelClassEditor;
    }());
    schema_editor.LabelClassEditor = LabelClassEditor;
})(schema_editor || (schema_editor = {}));
//# sourceMappingURL=label_class_editor.js.map